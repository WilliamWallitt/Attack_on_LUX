...
class Player:
    ...
    def __init__(self, *, player_id: int, agents: List[Unit], enemy_agents: List[Unit],
                 factories: List[Factory], enemy_factories: List[Factory], resources: List[Resource]):
        self.id = player_id
    ...
    def __get_factory(self, agent: Unit):
        return next((item for item in self.factories if item.position == agent.position and item.player == self.id), None)

    def __get_enemy_factory(self, agent: Unit):
        return next((item for item in self.enemy_factories if item.position == agent.position and item.player == self.id), None)
        ...
        for i, agent in enumerate(self.agents):
            ...
            if action_idx == 10:
                ...
                if factory is not None and enemy_agent is not None and enemy_factory is not None:
                    create_factory(self.id, agent.position, map_size=50)
    ...
    def choose_action(self, observation) -> np.ndarray:
        ...
        available_actions = [index for index, value in enumerate([None] * 11)]
    ...
    def is_game_over(self) -> bool:
        ...
        if num_alive_factories == 0 or (num_alive_agents == 0 and sum(x.health["amount"] for x in self.factories if x.health["amount"] > 6) == 0):
            return True
