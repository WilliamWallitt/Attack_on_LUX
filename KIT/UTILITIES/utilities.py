...
def produce_worker_unit(player: int, position: np.ndarray) -> Unit:
    return Unit(
        player=player,
...
def produce_warrior_unit(player: int, position: np.ndarray) -> Unit:
    return Unit(
        player=player,
...
def create_factory(player: int, position: np.ndarray, map_size: int) -> Factory:
    return Factory(
        player=player,
